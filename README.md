...
- **現在地検索**: 現在地から近い施設を検索
- **お気に入い機能**: LocalStorageを使用したお気に入り管理
...
このアプリケーションは、蝶と温泉（♨）を融合させた可愛らしいアイコンを採用しています。

![favicon](https://yokko405.github.io/tattoo-bath-app2/favicon.svg)

| サイズ | 用途 | ファイル |
|--------|------|---------|
| 64×64 | ブラウザタブ・デスクトップショートカット | `favicon.svg` |
| 192×192 | Android ホーム画面 | `android-chrome-192x192.png` |
| 512×512 | Android スプラッシュスクリーン | `android-chrome-512x512.png` |
| 180×180 | iPhone ホーム画面 | `apple-touch-icon.png` |

**デザイン**: 深い藍色の背景（`#1A3A5C`）に、温泉の湯気を表現した白いパス、その中に赤いグラデーション（`#E85D4A` → `#C44536`）の蝶を配置。タトゥーと温泉の融合を表現しています。
